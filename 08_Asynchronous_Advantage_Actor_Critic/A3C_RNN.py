"""
Asynchronous Advantage Actor Critic (A3C) + RNN with continuous action space, Reinforcement Learning
"""


import tensorflow as tf
import multiprocessing
import threading
import matplotlib.pyplot as plt
import numpy as np
import gym 
import os
import shutil

GAME = 'Pendulum-v0'
OUTPUT_GRAPH=True
LOG_DIR='./log'
N_WORKERS = multiprocessing.cpu_count()
MAX_GLOBAL_EP = 2000
MAX_EP_STEP = 200
GLOBAL_NET_SCOPE = 'GlobalNet'
UPDATE_GLOBAL_ITER = 10
GAMMA = 0.9
ENTROPY_BETA = 0.01
LR_A = 0.0001
LR_C = 0.001
GLOBAL_RUNNING_R = []
GLOBAL_EP = 0

RNN_CELL_SIZE = 64

env = gym.make(GAME)
N_S, N_A = env.observation_space.shape[0], env.action_space.shape[0]
A_BOUND = [env.action_space.low, env.action_space.high]
print(N_S, N_A, A_BOUND)
class A3CNet:
    def __init__(self, scope, globalA3C=None):
        if scope == GLOBAL_NET_SCOPE:
            with tf.variable_scope(scope):
                self.s = tf.placeholder(tf.float32, [None, N_S], 'S')
                self.a_params, self.c_params = self._build_net(scope)[-2:]
        else:
            with tf.variable_scope(scope):
                self.s = tf.placeholder(tf.float32,[None, N_S], 'S')
                self.a_his = tf.placeholder(tf.float32, [None,N_A], 'A')
                self.v_target = tf.placeholder(tf.float32, [None,1], 'Vtarget')

                mu, sigma, self.v, self.a_params, self.c_params = self._build_net(scope)

                td = tf.subtract(self.v_target, self.v, name='TD_error')
                with tf.name_scope('c_loss'):
                    self.c_loss = tf.reduce_mean(tf.square(td))

                with tf.name_scope('wrap_a_out'):
                    mu, sigma = mu*A_BOUND[1], sigma + 1e-4
                normal_dist = tf.distributions.Normal(mu, sigma)

                with tf.name_scope('a_loss'):
                    log_prob = normal_dist.log_prob(self.a_his)
                    exp_v = log_prob * tf.stop_gradient(td)
                    # encourage exploration, 类似于Q-Learning中随机选择还是选择最大值
                    entropy = normal_dist.entropy()
                    self.exp_v = ENTROPY_BETA * entropy + exp_v
                    self.a_loss = tf.reduce_mean(-self.exp_v)

                with tf.name_scope('choose_a'):
                    self.A = tf.clip_by_value(tf.squeeze(normal_dist.sample(1),axis=[0,1]), A_BOUND[0], A_BOUND[1])

                with tf.name_scope('local_grad'):
                    self.a_grads = tf.gradients(self.a_loss, self.a_params)
                    self.c_grads = tf.gradients(self.c_loss, self.c_params)
            
            with tf.name_scope('sync'):
                with tf.name_scope('pull'):
                    self.pull_a_params_op = [l_p.assign(g_p) for l_p, g_p in zip(self.a_params, globalA3C.a_params)]
                    self.pull_c_params_op = [l_p.assign(g_p) for l_p, g_p in zip(self.c_params, globalA3C.c_params)]

                with tf.name_scope('push'):
                    self.update_a_op = OPT_A.apply_gradients(zip(self.a_grads, globalA3C.a_params))
                    self.update_c_op = OPT_C.apply_gradients(zip(self.c_grads, globalA3C.c_params))

    def _build_net(self, scope):
        w_init = tf.random_normal_initializer(0., .1)

        with tf.variable_scope('critic'):
            cell_size = RNN_CELL_SIZE
            # [time_step, feature] -> [time_step, batch, feature] batach_size 为1
            s = tf.expand_dims(self.s, axis=1, name='timely_input')
            
            rnn_cell = tf.contrib.rnn.BasicRNNCell(cell_size)
            self.init_state = rnn_cell.zero_state(batch_size=1, dtype=tf.float32)
            outputs, self.final_state = tf.nn.dynamic_rnn(
                    cell=rnn_cell, inputs=s, initial_state=self.init_state,
                    time_major=True)
            #joined state representation [time_step, batch, feature] -> [-1, feature]
            cell_out = tf.reshape(outputs, [-1, cell_size], name='flatten_rnn_outputs')

            l_c = tf.layers.dense(cell_out, 100, tf.nn.relu6, kernel_initializer=w_init, name='lc')
            v = tf.layers.dense(l_c, 1, kernel_initializer=w_init, name='v')


        # state representation is based on critic
        with tf.variable_scope('actor'):
            l_a = tf.layers.dense(cell_out, 200, tf.nn.relu6, kernel_initializer=w_init, name='la')
            mu = tf.layers.dense(l_a, N_A, tf.nn.tanh, kernel_initializer=w_init, name='mu')
            sigma = tf.layers.dense(l_a, N_A, tf.nn.softplus, kernel_initializer=w_init, name='sigma')

        a_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope+'/actor')
        c_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope+'/critic')
        return mu, sigma, v, a_params, c_params

    def update_global(self, feed_dict):
        SESS.run([self.update_a_op, self.update_c_op], feed_dict)

    def pull_global(self):
        SESS.run([self.pull_a_params_op, self.pull_c_params_op])

    def choose_action(self, s, cell_state):
        s = s[np.newaxis,:]
        action, cell_state = SESS.run([self.A,self.final_state],{self.s:s,self.init_state:cell_state})
        return action, cell_state


class Worker:
    def __init__(self, name, globalA3C):
        self.env = gym.make(GAME).unwrapped
        self.name = name
        self.A3C = A3CNet(name, globalA3C)

    def work(self):
        global GLOBAL_RUNNING_R, GLOBAL_EP
        total_step = 1
        buffer_s, buffer_a, buffer_r = [], [], []
        while not COORD.should_stop() and GLOBAL_EP < MAX_GLOBAL_EP:
            s = self.env.reset()
            ep_r = 0
            # zero rnn state at begining 
            rnn_state = SESS.run(self.A3C.init_state)
            # keep rnn state for updating global net
            keep_state = rnn_state.copy()
            for ep_t in range(MAX_EP_STEP):
                a, rnn_state_ = self.A3C.choose_action(s, rnn_state)
                s_, r, done, info = self.env.step(a)
                done = True if ep_t == MAX_EP_STEP - 1 else False

                ep_r += r
                buffer_s.append(s)
                buffer_a.append(a)
                buffer_r.append((r+8)/8) # normalize
                
                # update global and assign to local net
                if total_step % UPDATE_GLOBAL_ITER == 0 or done:
                    if done:
                        v_s_ = 0
                    else:
                        v_s_ = SESS.run(self.A3C.v, {self.A3C.s:s_[np.newaxis,:], self.A3C.init_state: rnn_state})[0,0]

                    buffer_v_target = []
                    # 进行 n_steps forward view
                    for r in buffer_r[::-1]:
                        v_s_ = r + GAMMA*v_s_
                        buffer_v_target.append(v_s_)
                    buffer_v_target.reverse()

                    buffer_s, buffer_a, buffer_v_target = np.vstack(buffer_s), np.vstack(buffer_a), np.vstack(buffer_v_target)
                    feed_dict = {
                        self.A3C.s : buffer_s,
                        self.A3C.a_his: buffer_a,
                        self.A3C.v_target:buffer_v_target,
                        self.A3C.init_state: keep_state,
                    }
                    self.A3C.update_global(feed_dict)
                    buffer_s, buffer_a, buffer_r = [], [], []
                    self.A3C.pull_global() 
                    # replace the keep_state as the initial rnn_stat_
                    keep_state = rnn_state_.copy()

                s = s_
                total_step += 1
                # rennew rnn state
                rnn_state = rnn_state_
                if done:
                    if  len(GLOBAL_RUNNING_R) == 0:
                        GLOBAL_RUNNING_R.append(ep_r)
                    else:
                        GLOBAL_RUNNING_R.append(0.9*GLOBAL_RUNNING_R[-1] + 0.1*ep_r)
                    print(self.name,
                        "Ep:", GLOBAL_EP,
                        "| Ep_r: %i"%GLOBAL_RUNNING_R[-1])
                    GLOBAL_EP += 1
                    break

if __name__ == "__main__":
    SESS = tf.Session()

    with tf.device("/cpu:0"):
        OPT_A = tf.train.RMSPropOptimizer(LR_A, name='RMSPropA')
        OPT_C = tf.train.RMSPropOptimizer(LR_C, name='RMSPropC')
        GLOBAL_A3C = A3CNet(GLOBAL_NET_SCOPE)

        workers = []
        for i in range(N_WORKERS):
            i_name = 'W_%i'%i
            workers.append(Worker(i_name, GLOBAL_A3C))

    COORD = tf.train.Coordinator()
    SESS.run(tf.global_variables_initializer())

    if OUTPUT_GRAPH:
        if os.path.exists(LOG_DIR):
            shutil.rmtree(LOG_DIR)
        tf.summary.FileWriter(LOG_DIR, SESS.graph)

    worker_threads = []
    for worker in workers:
        job = lambda: worker.work()
        t = threading.Thread(target = job)
        t.start()
        worker_threads.append(t)
    COORD.join(worker_threads)

    plt.plot(np.arange(len(GLOBAL_RUNNING_R)), GLOBAL_RUNNING_R)
    plt.xlabel('step')
    plt.ylabel('Total moving reward')
    plt.show()

